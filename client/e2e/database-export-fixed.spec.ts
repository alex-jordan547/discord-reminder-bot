...
test.describe('Database Export - Fixed Implementation', () => {
  ...
  test('should export database in JSON format with correct filename and content', async ({ page }) => {
    // Select JSON format
    ...
    expect(download.suggestedFilename()).toMatch(/^discord_bot_database_export_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.json$/);
  ...
  test('should export database in CSV format with correct filename and content', async ({ page }) => {
    // Select CSV format
    ...
    // Verify filename format
    expect(download.suggestedFilename()).toMatch(/^discord_bot_database_export_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.csv$/);
  ...
  test('should export database in SQLite format with correct filename and content', async ({ page }) => {
    ...
    // Verify filename format
    expect(download.suggestedFilename()).toMatch(/^discord_bot_database_export_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.db$/);
  ...
  test('should show error when no format is selected', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="format-error"]')).toContainText('Veuillez sélectionner un format');
  });
  ...
  test('should generate unique filenames for multiple exports', async ({ page }) => {
    ...
    expect(download1.suggestedFilename()).toMatch(/^discord_bot_database_export_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.json$/);
    expect(download2.suggestedFilename()).toMatch(/^discord_bot_database_export_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.json$/);
  });
  ...
  test('should handle all formats correctly in sequence', async ({ page }) => {
    ...
    for (let i = 0; i < formats.length; i++) {
      ...
      expect(download.suggestedFilename()).toMatch(
        new RegExp(`^discord_bot_database_export_\\d{4}-\\d{2}-\\d{2}_\\d{2}-\\d{2}-\\d{2}\\.${extension}$`)
      );
