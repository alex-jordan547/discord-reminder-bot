...
export function useOfflineQueue(options: Partial<OfflineQueueOptions> = {}) {
   ...
   */
  function queueOperation(operation: Omit<QueuedOperation, 'id' | 'timestamp' | 'retries'>): QueuedOperation {
  ...
  async function processQueue() {
    ...
    if (successCount > 0) {
      if (addNotification) {
        ...
        console.log(`Sync complete: ${successCount} operations completed successfully${failedCount > 0 ? `, ${failedCount} failed` : ''}`)
      }
    }
  ...
  function getQueueStats() {
    const stats = {
      ...
      byMethod: queue.value.reduce((acc, op) => {
        acc[op.method] = (acc[op.method] || 0) + 1
        return acc
      }, {} as Record<string, number>),
      oldestOperation: queue.value.length > 0 
        ? Math.min(...queue.value.map(op => op.timestamp.getTime()))
        : null
    }
