...
describe('Dashboard Workflow Integration', () => {
  ...
  beforeEach(async () => {
    ...
    router = createRouter({
      ...
      routes: [
        { path: '/', name: 'overview', component: { template: '<div class="overview-view">Overview</div>' } },
        { path: '/metrics', name: 'metrics', component: { template: '<div class="metrics-view">Metrics</div>' } },
        { path: '/database', name: 'database', component: { template: '<div class="database-view">Database</div>' } },
        { path: '/alerts', name: 'alerts', component: { template: '<div class="alerts-view">Alerts</div>' } },
      ],
    });
  ...
  describe('Performance and Optimization', () => {
    ...
    it('should debounce rapid updates', async () => {
      ...
      expect(dashboardStore.metrics?.timestamp).toBe(
        new Date(Date.now() + 9).toISOString()
      );
    });
