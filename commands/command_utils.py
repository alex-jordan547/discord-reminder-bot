...
def create_health_embed(stats: Dict[str, Any]) -> discord.Embed:
    ...
    embed = discord.Embed(title="🏥 État de santé du bot", color=color, timestamp=datetime.now())

    # Indicateur d'état général
    embed.add_field(name="📊 État général", value=status_indicator, inline=True)
    ...
    if stats["most_common_errors"]:
        error_list = "\n".join(
            [f"• **{error}**: {count}" for error, count in stats["most_common_errors"]]
        )
        embed.add_field(
