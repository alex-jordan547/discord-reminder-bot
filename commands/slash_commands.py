...
class SlashCommands(commands.Cog):
    ...
    async def watch(self, interaction: discord.Interaction, message: str, interval: int = 60):
        ...
        # Validate interval
        interval = Settings.validate_interval_minutes(interval)
        ...
        try:
            ...
            embed.add_field(name="⏰ Intervalle", value=Settings.format_interval_display(interval), inline=True)
            ...
            logger.info(f"Added match {link_info.message_id} to watch list on guild {interaction.guild.id} with {interval}min interval")
...
def register_slash_commands(bot: commands.Bot) -> None:
    ...
    logger.info(f"Registered slash commands and loaded {len(watched_matches)} matches from storage")