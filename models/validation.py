            ...
            if hasattr(field, "rel_model") and field.rel_model:
                ...
                if value is not None:
                    ...
                    try:
                        ...
                        errors.append(
                            f"{field_name} references non-existent {field.rel_model.__name__}"
                        )
...
class SerializationMixin:
    ...
    def to_dict(
        self, include_relations: bool = False, exclude_fields: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
                ...
                if hasattr(field, "field_type") and "datetime" in str(field.field_type).lower():
                    if isinstance(field_value, str):
                        try:
                            ...
                            logger.warning(
                                f"Invalid datetime format for {field_name}: {field_value}"
                            )
