...
class DatabaseMigrator {
        ...
        try {
            ...
            if (sqliteData.count > 0) {
                ...
                for (let i = 0; i < sqliteData.data.length; i += batchSize) {
                    ...
                    this.log('INFO', `📝 Inserted ${Math.min(i + batchSize, sqliteData.data.length)}/${sqliteData.data.length} records`);
                }
        ...
        for (const table of tables) {
            try {
                ...
                if (verification[table].match) {
                    ...
                    this.log('WARN', `⚠️  ${table}: SQLite ${sqliteCount} vs PostgreSQL ${pgCount} (mismatch)`);
                }
            } catch (error) {
    ...
    async updateSequences() {
        ...
        const tables = ['users', 'guilds', 'events', 'event_participants', 'reminders', 'bot_stats', 'sessions'];
    ...
    async migrate() {
        ...
        try {
            ...
            const reportFile = path.join(this.backupDir, `migration-report-${this.migrationStartTime.toISOString().replace(/[:.]/g, '-')}.json`);
