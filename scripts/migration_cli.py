...
def cmd_cleanup(args):
    ...
    if not args.force:
        response = input(
            f"This will delete old backup files, keeping only {args.keep} most recent. Continue? (y/N): "
        )
        if response.lower() != "y":
...
Examples:
  ...
  %(prog)s cleanup                   # Clean up old backups
    ...
    diagnose_parser.add_argument(
        "--json-file", default="watched_reminders.json", help="Path to JSON file to analyze"
    )
    ...
    backup_parser.add_argument(
        "--json-file", default="watched_reminders.json", help="Path to JSON file to backup"
    )
    ...
    migrate_parser.add_argument(
        "--json-file", default="watched_reminders.json", help="Path to JSON file to migrate"
    )
    migrate_parser.add_argument(
    ...
    rollback_parser.add_argument(
        "--target", default="watched_reminders.json", help="Target path for restored JSON file"
    )
