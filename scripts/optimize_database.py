...
from utils.database_optimization import optimize_database, maintenance_cleanup
...
def main():
    ...
    if stats and 'error' not in stats:
        ...
        print(f"  Events: {stats.get('events', 0)} ({stats.get('active_events', 0)} active, {stats.get('paused_events', 0)} paused)")
        print(f"  Reactions: {stats.get('reactions', 0)}")
        print(f"  Reminder Logs: {stats.get('reminder_logs', 0)}")
        print(f"  Recent Activity (7 days): {stats.get('recent_events', 0)} events, {stats.get('recent_reactions', 0)} reactions")
