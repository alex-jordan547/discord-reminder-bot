...
class StagingValidator:
    ...
    async def generate_test_data(self) -> bool:
        ...
        try:
            ...
            self._log_test_result(
                "test_data_generation", True, f"Generated {len(test_data)} test events"
            )
            return True

        except Exception as e:
            self._log_test_result(
                "test_data_generation", False, f"Test data generation failed: {e}"
            )
            return False
    ...
    async def _verify_data_integrity(
        self, original_events: List[Dict], migrated_events: List[Dict]
    ) -> None:
        """Verify data integrity after migration."""
        # Create lookup dictionaries
        original_by_id = {
            str(event.get("message_id", event.get("id"))): event for event in original_events
        }
        migrated_by_id = {
            str(event.get("message_id", event.get("id"))): event for event in migrated_events
        }
    ...
    async def test_rollback_procedures(self) -> bool:
        ...
        try:
            ...
            if json_count != sqlite_count:
                raise Exception(
                    f"Event count mismatch after rollback: {sqlite_count} -> {json_count}"
                )
    ...
    async def test_error_recovery(self) -> bool:
        ...
        for scenario_name, test_func in test_scenarios:
            try:
                ...
                self._log_test_result(
                    f"error_recovery_{scenario_name}", False, f"Error recovery test failed: {e}"
                )
                all_passed = False
    ...
    async def _test_feature_flag_fallback(self) -> bool:
        ...
        try:
            ...
            feature_flags.trigger_fallback(FeatureFlag.SQLITE_STORAGE, "Staging test fallback")
    ...
    async def _test_concurrent_access_recovery(self) -> bool:
        ...
        try:
            ...
            critical_errors = [
                r for r in results if isinstance(r, Exception) and "critical" in str(r).lower()
            ]
    ...
    async def test_performance(self) -> bool:
        ...
        for test_name, test_func in performance_tests:
            try:
                ...
                self._log_test_result(f"performance_{test_name}", success, message, duration)
                ...
                self._log_test_result(
                    f"performance_{test_name}", False, f"Performance test failed: {e}"
                )
                all_passed = False
    ...
    async def _test_bulk_operations_performance(self) -> Tuple[bool, str]:
        ...
        threshold_ms = (
            self.config["performance_threshold_ms"] / 10
        )  # Lower threshold for individual ops

        success = avg_time_ms < threshold_ms
        metrics = (
            f"{bulk_time_ms:.2f}ms total, {avg_time_ms:.2f}ms avg (threshold: {threshold_ms}ms)"
        )
        ...
        try:
            ...
            if self.original_data_backup and os.path.exists(self.original_data_backup):
                ...
                json_backup = os.path.join(
                    self.original_data_backup, os.path.basename(Settings.REMINDERS_SAVE_FILE)
                )
                ...
                db_backup = os.path.join(
                    self.original_data_backup, os.path.basename(Settings.DATABASE_PATH)
                )
    ...
    def generate_validation_report(self) -> Dict:
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": (
                    len(successful_tests) / len(self.test_results) * 100 if self.test_results else 0
                ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if failed_tests:
            ...
            if any("migration" in name for name in failed_test_names):
                recommendations.append(
                    "Migration process needs improvement - check data integrity and error handling"
                )

            if any("rollback" in name for name in failed_test_names):
                recommendations.append(
                    "Rollback procedures need refinement - ensure reliable fallback mechanisms"
                )

            if any("performance" in name for name in failed_test_names):
                recommendations.append(
                    "Performance optimization required - consider indexing and query optimization"
                )
        ...
        if slow_tests:
            recommendations.append(
                "Some performance tests were slow - monitor performance in production"
            )
...
async def main():
    ...
    parser.add_argument(
        "--test-data-size", type=int, default=100, help="Number of test events to create"
    )
    ...
    parser.add_argument(
        "--skip-error-recovery", action="store_true", help="Skip error recovery testing"
    )
