...
export async function registerAuthRoutes(fastify: FastifyInstance) {
  ...
  fastify.delete('/api/auth/tokens/:tokenId', {
    preHandler: [requireAuth, requirePermission('admin')]
  }, async (request: FastifyRequest<{
  ...
  fastify.delete('/api/auth/sessions', {
    ...
    try {
      ...
      await auditLogger.logAuthenticationEvent(
        ...
        'logout',
        'success',
        { destroyedSessions: destroyedCount, logoutAll: true }
      );
  ...
  fastify.get('/api/files/:filename', {
    ...
    try {
      ...
      // Log file access
      await fileUploadService.logFileAccess(
        filename,
        user.userId,
        'download',
        {
