...
export function createTestDatabase(): TestDatabaseInstance {
  ...
  // Exécute les migrations ou crée les tables manuellement
  const migrationsPath = path.join(process.cwd(), '..', 'drizzle', 'migrations');
  try {
    migrate(db, { migrationsFolder: migrationsPath });
  } catch (error) {
    console.warn('Migrations not found or failed, creating tables manually:', error);
    // Crée les tables manuellement si les migrations échouent
  ...
  try {
    ...
    sqlite.exec(`
      CREATE TABLE IF NOT EXISTS events (
        ...
        users_who_reacted TEXT NOT NULL DEFAULT '[]',
    ...
    sqlite.exec(`
      CREATE TABLE IF NOT EXISTS reminder_logs (
        ...
        error TEXT,
        created_at INTEGER NOT NULL DEFAULT (unixepoch()),
        FOREIGN KEY (event_id) REFERENCES events (message_id)
      );
    `);

    console.log('✅ Tables created manually for tests');
  } catch (error) {
