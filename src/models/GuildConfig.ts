 ...
 */
export const DEFAULT_GUILD_CONFIG: Omit<GuildConfigData, 'guildId' | 'guildName' | 'createdAt' | 'updatedAt' | 'lastUsedAt'> = {
  // Channel defaults
...
export class GuildConfig extends BaseModel<GuildConfigData> {
  ...
  get guildId(): string { return this.data.guildId; }
  get guildName(): string { return this.data.guildName; }
  get reminderChannelId(): string | null { return this.data.reminderChannelId; }
  get adminRoleIds(): string[] { return this.data.adminRoleIds; }
  get defaultIntervalMinutes(): number { return this.data.defaultIntervalMinutes; }
  get autoDeleteEnabled(): boolean { return this.data.autoDeleteEnabled; }
  get autoDeleteDelayMinutes(): number { return this.data.autoDeleteDelayMinutes; }
  get maxMentionsPerReminder(): number { return this.data.maxMentionsPerReminder; }
  get useEveryoneAboveLimit(): boolean { return this.data.useEveryoneAboveLimit; }
  get defaultReactions(): string[] { return this.data.defaultReactions; }
  get timezone(): string { return this.data.timezone; }
  get delayBetweenRemindersMs(): number { return this.data.delayBetweenRemindersMs; }
  ...
  isValid(): boolean {
    try {
      ...
      if (typeof this.data.defaultIntervalMinutes !== 'number' || this.data.defaultIntervalMinutes < 0) {
        console.log('Invalid defaultIntervalMinutes:', this.data.defaultIntervalMinutes);
        return false;
      }
      if (typeof this.data.autoDeleteDelayMinutes !== 'number' || this.data.autoDeleteDelayMinutes < 0) {
        console.log('Invalid autoDeleteDelayMinutes:', this.data.autoDeleteDelayMinutes);
        return false;
      }
      if (typeof this.data.delayBetweenRemindersMs !== 'number' || this.data.delayBetweenRemindersMs < 0) {
        console.log('Invalid delayBetweenRemindersMs:', this.data.delayBetweenRemindersMs);
        return false;
      }
      
      // Validate limits
      if (typeof this.data.maxMentionsPerReminder !== 'number' || this.data.maxMentionsPerReminder < 0) {
      ...
      if (!this.data.createdAt || !this.data.updatedAt || !this.data.lastUsedAt) {
        console.log('Missing dates:', this.data.createdAt, this.data.updatedAt, this.data.lastUsedAt);
        return false;
      }
  ...
  getDisplayValues(): {
    ...
    return {
      ...
      adminRoles: this.data.adminRoleNames.length > 0 
