...
`;
  ...
  test('should access the dashboard endpoint', async ({ page }) => {
    ...
    try {
      ...
      try {
        const botLogs = execSync('docker compose logs discord-reminder-bot --tail=20', {
          encoding: 'utf8',
        });
  ...
  test('should verify database migration completed successfully', async () => {
    ...
    try {
      ...
      const tableQueries = [
        {
          name: 'users',
          query:
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'discord_bot' AND table_name = 'users'",
        },
        {
          name: 'guilds',
          query:
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'discord_bot' AND table_name = 'guilds'",
        },
        {
          name: 'events',
          query:
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'discord_bot' AND table_name = 'events'",
        },
      ];
      ...
      for (const tableQuery of tableQueries) {
        ...
        expect(
          result.rows.length,
          `Table '${tableQuery.name}' should have columns`,
        ).toBeGreaterThan(0);

        // Check for expected columns
        const columnNames = result.rows.map(row => row.column_name);
        expect(columnNames, `Table '${tableQuery.name}' should have id column`).toContain('id');
        expect(columnNames, `Table '${tableQuery.name}' should have created_at column`).toContain(
          'created_at',
        );
