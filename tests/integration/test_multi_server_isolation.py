...
class TestMultiServerIsolation:
    ...
    async def test_user_isolation_between_guilds(self, setup_sqlite_db):
        ...
        user1 = User.create(user_id=user_id, guild=guild1_obj, username="TestUser", is_bot=False)

        user2 = User.create(user_id=user_id, guild=guild2_obj, username="TestUser", is_bot=False)

        # Create reactions in both guilds
        Reaction.create(event=event1, user_id=user_id, emoji="✅")

        Reaction.create(event=event2, user_id=user_id, emoji="❌")

        # Verify isolation - each guild should only see its own user reactions
        guild1_reactions = list(
            Reaction.select().join(Event).join(Guild).where(Guild.guild_id == guild1_id)
        )
        guild2_reactions = list(
            Reaction.select().join(Event).join(Guild).where(Guild.guild_id == guild2_id)
        )

        assert (
            len(guild1_reactions) == 1
        ), f"Guild 1 should have 1 reaction, got {len(guild1_reactions)}"
        assert (
            len(guild2_reactions) == 1
        ), f"Guild 2 should have 1 reaction, got {len(guild2_reactions)}"
    ...
    async def test_concurrent_operations_different_guilds(self, setup_sqlite_db):
        ...
        assert (
            len(guild1_created) == 20
        ), f"Guild 1 should have created 20 events, got {len(guild1_created)}"
        assert (
            len(guild2_created) == 15
        ), f"Guild 2 should have created 15 events, got {len(guild2_created)}"
        assert (
            len(guild3_created) == 25
        ), f"Guild 3 should have created 25 events, got {len(guild3_created)}"
        ...
        assert (
            len(guild1_updated) == 10
        ), f"Guild 1 should have updated 10 events, got {len(guild1_updated)}"
        assert (
            len(guild2_updated) == 8
        ), f"Guild 2 should have updated 8 events, got {len(guild2_updated)}"
        assert (
            len(guild3_updated) == 12
        ), f"Guild 3 should have updated 12 events, got {len(guild3_updated)}"
        ...
        for events in [final_guild1_events, final_guild2_events, final_guild3_events]:
            ...
            assert guild_message_ids.isdisjoint(
                all_message_ids
            ), "Message ID collision between guilds"
    ...
    async def test_due_events_isolation(self, setup_sqlite_db):
        ...
        assert (
            len(all_due_events) == 12
        ), f"Total should be 12 due events, got {len(all_due_events)}"
    ...
    async def test_reaction_updates_isolation(self, setup_sqlite_db):
        ...
        # Create events in both guilds
        event1 = await sqlite_event_manager.create_event(
            guild_id=guild1_id, message_id=9000001, channel_id=100001, title="Guild 1 Reaction Test"
        )

        event2 = await sqlite_event_manager.create_event(
            guild_id=guild2_id, message_id=9000002, channel_id=200001, title="Guild 2 Reaction Test"
        )
    ...
    async def test_guild_deletion_isolation(self, setup_sqlite_db):
        ...
        assert (
            len(guild1_events_after) == 2
        ), f"Guild 1 should have 2 events, got {len(guild1_events_after)}"
        assert (
            len(guild2_events_after) == 3
        ), f"Guild 2 should still have 3 events, got {len(guild2_events_after)}"
...
class TestConcurrencyBetweenGuilds:
    ...
    async def test_high_concurrency_different_guilds(self, setup_sqlite_db):
        ...
        async def guild_operations(guild_id: int):
            ...
            operations_completed = {"created": 0, "updated": 0, "queried": 0, "deleted": 0}
        ...
        results = await asyncio.gather(
            *[guild_operations(guild_id) for guild_id in guild_ids], return_exceptions=True
        )
        ...
        for i, result in enumerate(results):
            assert not isinstance(
                result, Exception
            ), f"Guild {guild_ids[i]} operations failed: {result}"

        # Verify operation counts
        total_operations = {"created": 0, "updated": 0, "queried": 0, "deleted": 0}
        ...
        for guild_id in guild_ids:
            ...
            assert (
                len(guild_events) == expected_remaining
    ...
    async def test_stress_test_isolation(self, setup_sqlite_db):
            ...
            for cycle in range(5):
                ...
                for message_id in message_ids[: len(message_ids) // 2]:
                    await sqlite_event_manager.update_event(
                        message_id=message_id, title=f"Updated Stress Event {message_id}"
                    )
        ...
        start_time = asyncio.get_event_loop().time()
        results = await asyncio.gather(
            *[stress_operations(guild_id) for guild_id in guild_ids], return_exceptions=True
        )
        end_time = asyncio.get_event_loop().time()
        ...
        for i, result in enumerate(results):
            assert not isinstance(
                result, Exception
            ), f"Stress test failed for guild {guild_ids[i]}: {result}"
        ...
        logger.info(
            f"Stress test verified: {len(all_message_ids)} unique events across {num_guilds} guilds"
        )
