...
class TestGuildModel:
    ...
    def test_guild_settings_property(self, test_db):
        ...
        guild = Guild.create(
            guild_id=123456789012345678, name="Test", settings='{"key": "value", "number": 42}'
        )
    ...
    def test_guild_settings_invalid_json(self, test_db):
        ...
        guild = Guild.create(guild_id=123456789012345678, name="Test", settings="invalid json")
    ...
    def test_guild_active_events_count(self, test_db, sample_guild):
        ...
        # Create some events
        Event.create(
            message_id=111222333444555111,
            channel_id=222333444555666222,
            guild=sample_guild,
            title="Active Event 1",
            is_paused=False,
        )
        Event.create(
            message_id=333444555666777333,
            channel_id=444555666777888444,
            guild=sample_guild,
            title="Active Event 2",
            is_paused=False,
        )
        Event.create(
            message_id=555666777888999555,
            channel_id=666777888999000666,
            guild=sample_guild,
            title="Paused Event",
            is_paused=True,
        )
...
class TestUserModel:
    ...
    def test_user_creation(self, test_db, sample_guild):
        ...
        user = User.create(
            user_id=987654321098765432, guild=sample_guild, username="TestUser", is_bot=False
        )
    ...
    def test_user_string_representation(self, test_db, sample_guild):
        ...
        user = User.create(user_id=123456789012345678, guild=sample_guild, username="TestUser")
...
class TestEventModel:
    ...
    def test_event_user_tracking(self, test_db, sample_guild, sample_event):
        ...
        user3 = User.create(
            user_id=333444555666777333, guild=sample_guild, username="User3", is_bot=True
        )
    ...
    def test_event_computed_properties(self, test_db, sample_guild, sample_event):
        ...
        expected_time = sample_event.last_reminder + timedelta(
            minutes=sample_event.interval_minutes
        )
    ...
    def test_event_indexes(self, test_db, sample_guild):
        ...
        due_events = Event.select().where(Event.last_reminder < past_time)
    ...
    async def test_event_update_accessible_users(
        self, mock_logger, test_db, sample_guild, sample_event
    ):
...
class TestReactionModel:
    ...
    def test_reaction_creation(self, test_db, sample_event):
        ...
        reaction = Reaction.create(event=sample_event, user_id=987654321098765432, emoji="✅")
    ...
    def test_reaction_event_relationship(self, test_db, sample_event):
        ...
        reaction = Reaction.create(event=sample_event, user_id=123456789012345678, emoji="✅")
...
class TestReminderLogModel:
    ...
    def test_reminder_log_creation(self, test_db, sample_event):
        ...
        log = ReminderLog.create(event=sample_event, scheduled_at=scheduled_time)
    ...
    def test_reminder_log_mark_as_sent(self, test_db, sample_event):
        ...
        log = ReminderLog.create(event=sample_event, scheduled_at=datetime.now())
    ...
    def test_reminder_log_validation(self, test_db, sample_event):
        ...
        log = ReminderLog(event=sample_event, scheduled_at=datetime.now(), status="pending")
    ...
    def test_reminder_log_string_representation(self, test_db, sample_event):
        ...
        log = ReminderLog.create(event=sample_event, scheduled_at=datetime.now(), users_notified=3)
