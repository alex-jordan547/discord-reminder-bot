...
from utils.data_migration import (
    DataMigrationService, JSONDataValidator, JSONToSQLiteTransformer,
    MigrationError, MigrationResult
)
from utils.backup_rollback import (
    BackupManager, RollbackManager, PostMigrationValidator,
    BackupResult, RollbackResult, ValidationResult,
    create_backup, rollback_migration, validate_migration
)
from models.database_models import Guild, User, Event, Reaction, ALL_MODELS
...
class TestJSONToSQLiteTransformer:
    ...
    def test_guild_and_user_caching(self, test_db):
        ...
        json_data = {
            "123456789012345678": event_data_1,
            "234567890123456789": event_data_2
        }
...
class TestPostMigrationValidator:
    ...
    def test_validate_post_migration_missing_events(self, temp_dir, sample_json_data):
        ...
        with patch('utils.backup_rollback.initialize_models'), \
             patch('utils.backup_rollback.get_database') as mock_get_db:
    ...
    def test_large_json_data(self, test_db):
        ...
        for i in range(100):  # 100 events
            ...
            large_data[str(message_id)] = {
                ...
                "title": f"Event {i}",
                "interval_minutes": 60.0,
                "users_who_reacted": [base_id + 3000 + j for j in range(i % 5)],  # Variable reactions
