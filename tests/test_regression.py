...
class TestBasicEventOperations:
    ...
    async def test_update_event_functionality(self, setup_sqlite_db):
        ...
        success = await sqlite_event_manager.update_event(
            message_id=message_id, title="Updated Title"
        )
        ...
        success = await sqlite_event_manager.update_event(
            message_id=message_id, interval_minutes=120
        )
        ...
        success = await sqlite_event_manager.update_event(message_id=message_id, is_paused=True)
        ...
        success = await sqlite_event_manager.update_event(message_id=999999999, title="Should Fail")
    ...
    async def test_delete_event_functionality(self, setup_sqlite_db):
        ...
        # Create an event
        event = await sqlite_event_manager.create_event(
            guild_id=guild_id, message_id=message_id, channel_id=555666777, title="To Be Deleted"
        )
...
class TestDueEventsAndScheduling:
    ...
    async def test_due_events_detection(self, setup_sqlite_db):
        ...
        for case in test_cases:
            if case["should_be_due"]:
                assert (
                    case["message_id"] in due_message_ids
                ), f"Event {case['title']} should be due but wasn't detected"
            else:
                assert (
                    case["message_id"] not in due_message_ids
        ...
        assert (
            len(due_events) == expected_due_count
    ...
    async def test_reminder_marking(self, setup_sqlite_db):
        ...
        updated_event = await sqlite_event_manager.get_event(message_id)
        assert (
            updated_event.last_reminder > original_last_reminder
...
class TestReactionHandling:
    ...
    async def test_reaction_updates(self, setup_sqlite_db):
        ...
        mock_reaction1.users.return_value.__aiter__ = AsyncMock(
            return_value=iter([mock_user1, mock_bot_user])
        )
...
class TestStatisticsAndReporting:
    ...
    async def test_event_properties(self, setup_sqlite_db):
        ...
        await sqlite_event_manager.update_event(
            message_id, is_paused=False, last_reminder=datetime.now() - timedelta(minutes=30)
        )
...
class TestLegacyCompatibility:
    ...
    async def test_legacy_method_names(self, setup_sqlite_db):
        ...
        from models.database_models import Event as JSONEvent
        ...
        # Make the event due
        await sqlite_event_manager.update_event(
            message_id, last_reminder=datetime.now() - timedelta(minutes=90)
        )
...
class TestErrorHandling:
    ...
    async def test_concurrent_operations(self, setup_sqlite_db):
        ...
        results = await asyncio.gather(
            create_events(1000000, 10), create_events(2000000, 10), create_events(3000000, 10)
        )
...
class TestComplexScenarios:
    ...
    async def test_full_event_lifecycle(self, setup_sqlite_db):
        ...
        await sqlite_event_manager.update_event(
            message_id, last_reminder=datetime.now() - timedelta(minutes=150)
        )
    ...
    async def test_multi_guild_scenario(self, setup_sqlite_db):
        ...
        for i, message_id in enumerate(all_message_ids[:7]):  # First 7 events
            await sqlite_event_manager.update_event(
                message_id, last_reminder=current_time - timedelta(minutes=90)
            )
