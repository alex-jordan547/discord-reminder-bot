...
from models.database_models import Guild, User, Event, Reaction, initialize_models, ALL_MODELS
...
class BackupManager:
    ...
    def create_manual_backup(self, json_file_path: str, backup_name: Optional[str] = None) -> BackupResult:
        """
        ...
        try:
            ...
            if backup_name:
                ...
                backup_filename = f"manual_{backup_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            else:
...
class RollbackManager:
    ...
    def rollback_to_json(self, backup_path: str, target_json_path: str = "watched_reminders.json") -> RollbackResult:
        """
        ...
        try:
            ...
            logger.info(f"Rollback completed successfully: restored {target_json_path} from {backup_path}")
    ...
    def get_rollback_candidates(self) -> List[Dict[str, Any]]:
        ...
        candidates = [
            ...
            if not backup['filename'].startswith('rollback_backup_')
        ]
...
class PostMigrationValidator:
    ...
    def validate_post_migration(self, original_json_path: str) -> ValidationResult:
        ...
        try:
            ...
            total_items = (result.comparison_stats['events_compared'] + 
                          result.comparison_stats['reactions_compared'])
            matched_items = (result.comparison_stats['events_matched'] + 
            ...
            if len(result.errors) == 0 and result.data_integrity_score >= 95.0:
                result.success = True
                logger.info(f"Post-migration validation passed with {result.data_integrity_score:.1f}% integrity")
            else:
                logger.warning(f"Post-migration validation issues found, integrity: {result.data_integrity_score:.1f}%")
...
def create_backup(json_file_path: str = "watched_reminders.json", backup_name: Optional[str] = None) -> BackupResult:
    """
...
def rollback_migration(backup_path: str, target_json_path: str = "watched_reminders.json") -> RollbackResult:
    """
