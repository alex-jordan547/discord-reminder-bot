...
class JSONToSQLiteTransformer:
    ...
    ) -> Tuple[List[Event], List[Reaction], MigrationResult]:
        """
        ...
        try:
            for message_id_str, event_data in json_data.items():
                try:
                    ...
                    event, event_reactions = self._transform_single_event(
                        message_id_str, event_data
                    )
                    if event:
                        events.append(event)
    ...
    def _transform_single_event(
        self, message_id_str: str, event_data: Dict[str, Any]
    ) -> Tuple[Optional[Event], List[Reaction]]:
        """
        ...
        try:
            ...
            last_reminder = self._parse_datetime(event_data.get("last_reminder"), datetime.now())
            created_at = self._parse_datetime(event_data.get("created_at"), datetime.now())
...
class DataMigrationService:
        ...
        try:
            if not os.path.exists(self.json_file_path):
                raise MigrationError(
                    "JSON_NOT_FOUND", f"JSON file not found: {self.json_file_path}"
                )
    ...
    def _save_to_database(
        self, events: List[Event], reactions: List[Reaction], transformer: JSONToSQLiteTransformer
    ) -> MigrationResult:
        """
    ...
    def validate_migration(self, json_file_path: Optional[str] = None) -> MigrationResult:
        ...
        try:
            ...
            if json_event_count != db_event_count:
                result.add_error(
                    f"Event count mismatch: JSON has {json_event_count}, DB has {db_event_count}"
                )
            else:
            ...
            for message_id_str, json_event in json_data.items():
                ...
                try:
                    ...
                    if (
                        abs(db_event.interval_minutes - json_event.get("interval_minutes", 60.0))
                        > 0.001
                    ):
                    ...
                    if json_reaction_count != db_reaction_count:
                        data_mismatches.append(
                            f"Event {message_id}: reaction count mismatch (JSON: {json_reaction_count}, DB: {db_reaction_count})"
                        )
...
async def check_and_migrate_if_needed() -> bool:
    """
    Check if JSON files exist and migrate them to SQLite if needed.

    Returns:
        bool: True if migration was performed, False if no migration needed
    """
    json_files = ["watched_reminders.json", "data/watched_reminders.json", "reminders.json"]

    # Check if any JSON files exist
    json_file_found = None
    for json_file in json_files:
        if os.path.exists(json_file):
            json_file_found = json_file
            break

    if not json_file_found:
        logger.info("No JSON files found, no migration needed")
        return False
    ...
    # Perform migration
    logger.info(f"Found JSON file {json_file_found}, starting migration...")
    migration = DataMigrationService(json_file_found)

    try:
        result = migration.migrate_from_json()
        if result.success:
            logger.info(f"Migration successful: {result.stats['events_migrated']} events migrated")
            return True
        else:
