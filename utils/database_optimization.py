...
from models.database_models import Event, Guild, Reaction, ReminderLog, User
...
class DatabaseOptimizer:
    ...
    def cleanup_old_reminder_logs(days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            deleted_count = (
                ReminderLog.delete().where(ReminderLog.created_at < cutoff_date).execute()
            )
    ...
    def get_database_statistics() -> Dict[str, Any]:
        ...
        try:
            ...
            # Events by guild
            guild_events = Event.select(
                Event.guild, fn.COUNT(Event.message_id).alias("count")
            ...
            stats["recent_reactions"] = (
                Reaction.select().where(Reaction.created_at >= week_ago).count()
            )
...
class QueryOptimizer:
            ...
            due_events = (
                ...
                .where(
                    ...
                    & (
                        fn.datetime(
                            Event.last_reminder,
                            "+" + fn.CAST(Event.interval_minutes, "TEXT") + " minutes",
                        )
                        <= current_time
                    )
                )
    ...
    def get_guild_events_with_stats(guild_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            events_with_stats = (
                Event.select(Event, fn.COUNT(Reaction.id).alias("reaction_count"))
