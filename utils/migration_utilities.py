...
from models.database_models import Guild, User, Event, Reaction, ReminderLog, initialize_models, ALL_MODELS
from persistence.database import get_database
from utils.data_migration import DataMigrationService, MigrationResult, MigrationError
from utils.backup_rollback import BackupManager, RollbackManager, PostMigrationValidator
...
class MigrationDiagnostics:
    ...
    def run_full_diagnostics(self, json_file_path: str = "watched_reminders.json") -> Dict[str, Any]:
        """
    ...
    def _analyze_json_structure(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in all_fields:
            ...
            if coverage < 100:
                structure['potential_issues'].append(f"Field '{field}' missing in {total_events - field_counts[field]} events")
    ...
    def _run_database_integrity_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            orphaned_reactions = Reaction.select().join(Event, join_type='LEFT OUTER').where(Event.id.is_null())
            ...
            orphaned_users = User.select().join(Guild, join_type='LEFT OUTER').where(Guild.guild_id.is_null())
            ...
            orphaned_events = Event.select().join(Guild, join_type='LEFT OUTER').where(Guild.guild_id.is_null())
            ...
            if total_events > 0:
                ...
                checks['data_consistency'].append(f"Average reactions per event: {avg_reactions_per_event:.2f}")
    ...
    def _check_migration_readiness(self, json_file_path: str) -> Dict[str, Any]:
        ...
        if not requirements['json_file_exists']:
            ...
            readiness['blocking_issues'].append("JSON file is not readable or contains invalid JSON")
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if performance.get('estimated_migration_time', 0) > 300:  # > 5 minutes
            recommendations.append("Large dataset detected - consider running migration during low-usage period")
...
class MigrationMetrics:
    ...
    def track_phase(self, phase_name: str, start_time: datetime, end_time: datetime, 
        ...
        logger.info(f"Tracked phase '{phase_name}': {phase_data['duration_seconds']:.2f}s, success: {success}")
...
class MigrationAdminCommands:
    ...
    def backup(self, json_file_path: str = "watched_reminders.json", backup_name: str = None) -> Dict[str, Any]:
        """Create a backup."""
        if backup_name:
    ...
    def migrate(self, json_file_path: str = "watched_reminders.json", 
        ...
        try:
            ...
            if create_backup:
                ...
                self.metrics.track_phase('backup', backup_start, backup_end, 
                                       backup_result.success, {'backup_path': backup_result.backup_path})
            ...
            self.metrics.track_phase('migration', migration_start, migration_end,
                                   migration_result.success, migration_result.stats)
            ...
            if migration_result.success:
                ...
                self.metrics.track_phase('validation', validation_start, validation_end,
                                       validation_result.success, validation_result.comparison_stats)
    ...
    def rollback(self, backup_path: str, target_json_path: str = "watched_reminders.json") -> Dict[str, Any]:
    ...
    def get_metrics(self, limit: int = 10) -> Dict[str, Any]:
        """Get historical migration metrics."""
        return {
            'historical_metrics': self.metrics.get_historical_metrics(limit)
        }
...
def perform_migration(json_file_path: str = "watched_reminders.json", 
                     create_backup: bool = True) -> Dict[str, Any]:
...
def perform_rollback(backup_path: str, target_json_path: str = "watched_reminders.json") -> Dict[str, Any]:
