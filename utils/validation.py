    ...
    try:
        ...
        if not isinstance(message_id, int):
            raise ValidationError(
                "❌ L'ID du message doit être un nombre entier", f"Type reçu: {type(message_id)}"
            )

        if message_id <= 0:
            raise ValidationError(
                "❌ L'ID du message doit être un nombre positif", f"Valeur reçue: {message_id}"
            )
        ...
        raise ValidationError(
            "❌ L'ID du message doit être un nombre valide", f"Erreur de conversion: {str(e)}"
        ) from e
...
async def validate_message_link(
    bot: commands.Bot, link: str, user: discord.User, require_permissions: bool = True
) -> MessageLinkInfo:
    """
    ...
    try:
        ...
        raise ValidationError("Erreur lors de la récupération du message", f"HTTP error: {str(e)}")
...
def validate_interval_minutes(interval: Union[int, str, float], test_mode: bool = False) -> int:
    ...
    try:
        ...
        if interval < 0:
            raise ValidationError(
                "❌ L'intervalle ne peut pas être négatif", f"Valeur reçue: {interval}"
            )
        ...
        raise ValidationError(
            "❌ L'intervalle doit être un nombre valide", f"Erreur de conversion: {str(e)}"
        ) from e
...
def validate_environment_config() -> List[str]:
    ...
    if separate_channel == "true":
        ...
        if not channel_name.strip():
            errors.append(
                "REMINDER_CHANNEL_NAME ne peut pas être vide si USE_SEPARATE_REMINDER_CHANNEL=true"
            )
...
def get_validation_error_embed(
    error: ValidationError, title: str = "Erreur de validation"
) -> discord.Embed:
    """
    ...
    """
    embed = discord.Embed(title=title, description=error.message, color=discord.Color.red())

    if error.technical_reason and os.getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
        embed.add_field(
            name="🔧 Détail technique (mode debug)", value=error.technical_reason, inline=False
        )
